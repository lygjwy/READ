...
from datasets import get_dataset_info, get_hybrid_auxiliary_dataloader
from models import get_classifier_rot
from trainers import get_classifier_hybrid_auxiliary_trainer
...
def main(args):
    ...
    get_dataloader_default = partial(
        get_hybrid_auxiliary_dataloader,
        root=args.data_dir,
        name=args.dataset,
        transform=transform,
    ...
    trainer = get_classifier_hybrid_auxiliary_trainer(classifier, train_loader, optimizer, scheduler)
    ...
    start_epoch = 1
    ori_cla_best_acc, rec_cla_best_acc = 0.0, 0.0
    ori_cla_best_state, rec_cla_best_state, last_state = {}, {}, {}
    
    for epoch in range(start_epoch, args.epochs+1):
        
        trainer.train_epoch()
        val_metrics = evaluator.eval_hybrid_auxiliary_classification(val_loader)
        
        ori_cla_best = val_metrics['test_ori_cla_acc'] > ori_cla_best_acc
        ori_cla_best_acc = max(val_metrics['test_ori_cla_acc'], ori_cla_best_acc)
        
        rec_cla_best = val_metrics['test_rec_cla_acc'] > rec_cla_best_acc
        rec_cla_best_acc = max(val_metrics['test_rec_cla_acc'], rec_cla_best_acc)
        ...
        if epoch == args.epochs:
            last_state = {
                ...
                'ori_cla_acc': val_metrics['test_ori_cla_acc'],
                'ori_rot_acc': val_metrics['test_ori_rot_acc'],
                'rec_cla_acc': val_metrics['test_rec_cla_acc'],
                'rec_rot_acc': val_metrics['test_rec_rot_acc']
            }
    ...
    print('---> Best ori cla acc: {:.4f}% | rec cla acc: {:.4f}%'.format(
